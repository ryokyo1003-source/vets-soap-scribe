...
var AI = {
  ...
  generateSOAP: async function (fullText, patient, refData, mode, apiKey) {
    ...
    var correctionRules = '# 音声認識テキストの補正ルール（必ず適用すること）\n'
      ...
      + '  真っすい/マッスイ → 「麻酔」\n'
      + '  後進/こうしん → 「口唇」（口の話題の場合）\n'
      + '  政権/せいけん → 「歯茎/歯肉」（口腔の話題の場合）\n'
      + '  ごサイプ/ゴサイク → 「毛玉」または「ゴワゴワ」（被毛の話題の場合）\n'
      + '  アガる/アゲー → 文脈に応じて判断（口が開く等）\n'
    ...
    if (mode === 'soap') {
      basePrompt = 'あなたは獣医師アシスタントです。音声認識された会話テキストからカルテを作成してください。\n'
        ...
        + '- 推測・一般論・「正常」「問題なし」などの補完は禁止。\n'
        + '- 食欲/元気/排尿/排便/嘔吐 → 会話に出た項目のみ記入。言及なしなら空欄のまま。\n'
        + '- [O]の所見 → 実際に話された検査結果・身体所見のみ。「動揺歯なし」「口臭なし」等、話にない否定所見を勝手に書かない。\n'
        ...
        + '  ✅ 「・フィラリア検査陰性（前日実施）」のように飼い主からの情報のみ簡潔に。\n'
