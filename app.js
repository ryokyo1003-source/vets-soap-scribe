...
var SOAP = {
  ...
  detectSpecialty: function (text, species) {
    var extra = '';
    species = (species || '').toLowerCase();

    // 鳥・エキゾチック動物の検出（種別 or 会話内容から）
    if (species === '鳥' || /鳥|インコ|オウム|文鳥|セキセイ|オカメ|ヨウム|九官鳥|フィンチ|爪切り.*鳥|羽|嘴|クチバシ|そのう|総排泄腔/.test(text)) {
      extra += '\n# 鳥類（エキゾチック）診察 — 必須の音声認識補正ルール\n'
        + 'この患者は鳥です。以下の誤変換を必ず修正すること：\n'
        + '- 発症/発生 → 「発情」（鳥の繁殖行動）\n'
        + '- 丹尿/短尿 → 「多尿」（鳥は発情すると多尿になる）\n'
        + '- 散乱/参卵 → 「産卵」\n'
        + '- リューブリン/リューブリング → 「リュープリン」（GnRHアゴニスト、発情抑制に使用）\n'
        + '- 肉ずり/肉づり → 「肉付き」（ボディコンディション）\n'
        + '- 津貝/ツカイ → 「ツガイ」（ペア）\n'
        + '- シート → 「シード」（鳥の餌、種子食）\n'
        + '- ペレット、シード、補助食、ネクトン、ビタミン剤は鳥の食餌用語\n'
        + '- そのう（素嚢）、総排泄腔、蝋膜、風切羽は鳥の解剖用語\n\n'
        + '# 鳥類カルテの記載ルール\n'
        + '- 体重はグラム(g)で記載（例: BW: 79g）。kgに変換しないこと。\n'
        + '- [S]は飼い主の主訴・報告のみ。[O]に獣医師の診察所見・体重・ボディコンディション。\n'
        + '- 発情関連: 産卵歴、多尿、巣作り行動、吐き戻し行動の有無を記載。\n';
    }
...
var AI = {
  ...
  transcribe: async function (audioBlob, apiKey, customDict) {
    ...
    var prompt = '獣医師の診察会話です。'
      ...
      + '処置：スケーリング、歯石除去、抜歯、皮下輸液、細胞診、スクレーピング、フィラリア検査、麻酔、鎮静。'
  ...
  generateSOAP: async function (fullText, patient, refData, mode, apiKey) {
    var specialtyExtra = SOAP.detectSpecialty(fullText, patient ? patient.species : '');
    ...
    var correctionRules = '# 音声認識テキストの補正ルール（必ず適用すること）\n'
      ...
      + '- 前後の文脈と無関係に唐突に出現する単語やフレーズは誤認識の可能性が高い。文脈に合う語に置換するか削除すること\n'
      + '- 獣医療用語の誤変換を正しい用語に修正して記載すること：\n'
      + '  べん/便 → 弁（心臓の話題）、僧帽便→僧帽弁、三尖便→三尖弁\n'
      + '  人体/仁帯 → 靭帯（関節の話題）、前十字人体→前十字靭帯\n'
      + '  反応性化形成 → 反応性過形成\n'
      + '  検治/点鼻検治 → 点鼻（鼻に薬を入れる話題）\n'
      + '  ウェブライザー/エレモライザー → ネブライザー\n'
      + '  毛虫腑/毛中部/盲腸分 → 盲腸便（ウサギの話題）\n'
      + '  類幹/涙幹/流感 → 涙管（目の話題）\n'
      + '  肺炎 → 歯肉炎（歯・口腔の話題の場合）\n'
      + '  鼻炎 → 膵炎（嘔吐・消化器の話題の場合）\n'
      ...
      + '  虚勢 → 去勢、避任 → 避妊\n'
      + '  真っすい/マッスイ/間瀬 → 麻酔\n'
      + '  感想/幹層 → 肝臓、神蔵/人造 → 腎臓、非蔵 → 脾臓\n'
      + '  暴行 → 膀胱、採決 → 採血、天敵 → 点滴\n'
      + '  応答 → 嘔吐（吐く話題）、応弾/横断 → 黄疸（肝臓話題）\n'
      + '  低酵素/定酵素/ていこうそ → 天疱瘡（皮膚・自己免疫の話題）\n'
      + '  デモリューション/でもリューション → レボリューション（駆虫・予防薬）\n'
      + '  クラミプラミン/クラミン → クロミプラミン（行動修正薬）\n'
    ...
    if (mode === 'soap') {
      basePrompt = 'あなたは獣医師アシスタントです。音声認識された会話テキストからカルテを作成してください。\n'
        ...
        + correctionRules
        + '# 最重要ルール：会話に出ていない情報は絶対に書かない\n'
        + '- 会話中に明示的に言及された事実のみ記載すること。\n'
        + '- 推測・一般論・「正常」「問題なし」「なし言及」「言及なし」などの補完は禁止。\n'
        + '- 「〜した」「〜を実施」と書く場合、会話中で実際にその行為が行われたことが明確に述べられている場合のみ記載すること。「〜が必要」「〜しないと」等の発言は将来の計画や一般論であり、実施した事実ではない。\n'
        + '- 食欲/元気/排尿/排便/嘔吐 → 会話に出た項目のみ記入。言及がない項目は空欄のまま。\n'
        + '- [O]の所見 → 実際に実施された検査結果・身体所見のみ。「〜が必要」等の会話中の発言を所見として記載しない。話にない否定所見を勝手に書かない。\n'
        + '- [P]の計画 → 実際に提案・合意された内容のみ。「指導」「推奨」等の一般的助言を追加しない。\n'
        + '- 意味が通らない文・文脈に無関係な記述は無視すること。無理に解釈してカルテに含めない。\n\n'
        + '# カルテ作成ルール\n'
        + '- テンプレート説明文は出力しない。実際の内容のみ記載。\n'
        + '- 箇条書きの行頭に「・」や「▪」などの記号をつけないこと。改行で項目を区切ること。\n'
        + '- 情報がない項目は一切出力しないこと。\n'
        + '- [S]1行目は「主訴：〇〇」の形式で来院理由を端的に1行で書くこと。\n'
        + '- [S]の「問診時」は簡潔な項目を改行区切りでまとめること。\n'
        + '  長文の説明・会話の要約文・獣医師の説明内容は禁止。\n'
        + '  飼い主からの情報のみ簡潔に。\n'
        + '- [S]には飼い主の主訴・報告のみ記載。獣医師の所見・判断・説明は[O][A][P]に振り分けること。\n'
        + '- [A]は診断名・鑑別診断・病態の評価のみ。治療方針や処置内容は[P]に書く。「何の病気か」だけを書き「何をするか」は書かない。\n'
      ...
      basePrompt = 'あなたは獣医師アシスタントです。音声認識された受付での問診・ヒアリング内容を[S]にまとめてください。\n'
        + '出力はJSON形式 { "soap_text": "...", "full_log": "..." } で行ってください。\n'
        + '- full_log: 入力テキストの内容を一切省略・要約せず、全文をそのまま収録すること。\n\n'
        + correctionRules
        + '# 最重要ルール：会話に出ていない情報は絶対に書かない\n'
        + '- 会話中に明示的に言及された事実のみ記載すること。\n'
        + '- 推測・一般論・「正常」「問題なし」「なし言及」「言及なし」などの補完は禁止。\n'
        ...
        + '- [S]の「問診時」は簡潔な項目を改行区切りでまとめること。長文禁止。\n'
