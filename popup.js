  ...
  async function processWithAI(audioBlob, apiKey, refData, selectedMode) {
    try {
      ...
      const whisperPrompt = `獣医師の診察会話です。`
        ...
        + `処置：スケーリング、歯石除去、抜歯、皮下輸液、細胞診、スクレーピング、涙管洗浄。`
        + `ウサギ：盲腸便、涙管、鼻涙管、不正咬合、毛球症、胃腸うっ滞、ペレット、牧草、チモシー。`
...
- 「牧草」の誤認識（「墓層」「幕僧」等）→「牧草」：ウサギ・ペレット・チモシーの話題 → 牧草`;
      ...
      - [S]セクションの構成ルール：
        ...
        「食欲：…嘔吐：」の一覧表には、会話中で明確に言及された情報のみを記入すること。会話に言及がない項目は空欄のままにし、「正常」「問題なし」等と推測で埋めないこと。
      ...
      if (selectedMode === 'interview') {
        ...
        - [S]セクションの構成ルール：
          ...
          一覧表には会話中で明確に言及された情報のみを記入し、言及がない項目は空欄のままにすること。「正常」「問題なし」等と推測で埋めないこと。
